...
class create_button:
    ...
    def draw_fixed_button(self):
        self.bt.configure(command=NONE, bg="#9ED9CC",
        ...
        def onKeyPress(event=None):
            ...
            if inputs in "123456789":
                ...
                if solve(self.board).check_v2(self.x, self.y, inputs):
                    ...
                    self.bt.bind(
                        "<Enter>", lambda _: self.bt.configure(bg="light yellow"))
                    self.bt.bind(
                        "<Leave>", lambda _: self.bt.configure(bg="white"))
                    self.root.unbind('<KeyPress>')
                else:
                    messagebox.showwarning(
                        "Existing Warning", "This number already exists, please try another one")
            else:
...
def create_button_list(dim, canvas, root, board):
    ...
    for x, i in enumerate(range(4, dim - (dim // 9), dim // 9)):
        ...
        for y, j in enumerate(range(4, dim - (dim // 9), dim // 9)):
            frames_list.append(
                Frame(root, width=width, height=width, bg="#FFDDE2"))
            frames_list[ct].propagate(False)

            num = str(board[x, y]) if board[x, y] != 0 else ''
            if num == '':
                # if num = 0, create input able button
                btn_list[x].append(create_button(
                    frames_list[ct], x, y, root, num, board, canvas, i, j, ct))
            ...
            canvas_id.append(canvas.create_window(
                j + 5, i + 5, anchor="nw", window=frames_list[ct]))
...
def func_buttons(canvas1, root, back_up, board, b_lst):
    ...
    def reset(prompt=True, shuffle=False):
        cond = False
        if prompt:
            cond = messagebox.askokcancel(
                "Prompt reset", "Are you sure to reset the puzzle? You will lose everything!")
                ...
                for i in range(9):
                    for j in range(9):
                        if len(str(board[i, j])) > 1:
                            ...
                            b_lst[i][j].get_button().configure(
                                text=board[i, j], command=NONE, bg="#FFDDE2", activebackground="#FFDDE2")
                            b_lst[i][j].get_button().unbind("<Enter>")
                            b_lst[i][j].get_button().unbind("<Leave>")
                if np.array_equal(board, back_up_prev):
                    messagebox.showinfo(
                        "No hints available", "Sorry, the current method can't give you any hint")
                else:
...
def main():
    ...
    c = Canvas(root, height=dimension, width=dimension,
               bg="black", bd=0, highlightthickness=0)
    img_bg = ImageTk.PhotoImage(Image.open("src\\Board_background.jpg").resize(
    ...
    c1 = Canvas(root, height=dimension, width=250,
